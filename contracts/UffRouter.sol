...
import './interfaces/IUffRouter02.sol';
...
contract UffRouter is IUffRouter02 {
    ...
    // UFF MOD - ADD BLACKLIST UTILS
    function blackList(address _token) public {
        require(IUffFactory(factory).feeToSetter() == msg.sender, 'UFF ROUTER: ONLY FEE TO SETTER');
        require(!isBlacklisted[_token], "UFF ROUTER: ALREADY BLACKLISTED");
        isBlacklisted[_token] = true;
    }

    function removeFromBlacklist(address _token) public {
        require(IUffFactory(factory).feeToSetter() == msg.sender, 'UFF ROUTER: ONLY FEE TO SETTER');
        require(isBlacklisted[_token], "UFF ROUTER: ALREADY WHITELISTED");
        isBlacklisted[_token] = false;
    }

    function blackListed(address _token) public view returns (bool) {
        return isBlacklisted[_token];
    }
    ...
    function _swap(uint[] memory amounts, address[] memory path, address _to) internal virtual {
        for (uint i; i < path.length - 1; i++) {
            ...
            // UFF MOD - BLACKLIST CHECK TOKEN A & TOKEN B
            require(!isBlacklisted[input], "UFF ROUTER: TOKEN A ADDR BLACKLISTED");
            require(!isBlacklisted[output], "UFF ROUTER: TOKEN B ADDR BLACKLISTED");
